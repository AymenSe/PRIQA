...
def extract_metric(dataset, device, metric="psnr", model=None, pref_dir="kadid_jpeg/PRefs"):
    """
...
def main():
    ...
    for metric_name in ["brisque", "psnr", "ssim", "lpips"]:
        ...
        X_train_pd, X_train_rd, y_train = extract_metric(train_dataset, device, metric=metric_name, model=model)
        X_test_pd, X_test_rd, y_test = extract_metric(test_dataset, device, metric=metric_name, model=model)
